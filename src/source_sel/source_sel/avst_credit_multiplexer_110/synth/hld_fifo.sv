...
module hld_fifo #(
    ...
    parameter int WIDTH = 32,                        // width of data path, at least 0
...
(
    ...
    `endif
    initial begin
    if ((STYLE != "hs") && (STYLE != "ms") && (STYLE != "ll") && (STYLE != "llreg") && (STYLE != "llram") && (STYLE != "zl") && (STYLE != "zlreg") && (STYLE != "zlram")) begin
        $fatal(1, "hld_fifo: illegal value of STYLE = %s, legal values are \"hs\", \"ms\", \"ll\", \"llreg\", \"llram\", \"zl\", \"zlreg\", or \"zlram\"\n", STYLE);
    end

    if (MAX_SLICE_WIDTH > 0 && (VALID_IN_OUTSIDE_REGS > 0 || STALL_IN_OUTSIDE_REGS > 0)) begin
        //currently expecting width slicing to only happen at the highest level of hld_fifo, and expecting extra valid/stall regs to be instantiated only as part of width-slicing,
        $fatal(1, "hld_fifo: did not expect outside stall/valid regs to be present if width-slicing is enabled"); 
    end
    end
    `ifdef ALTERA_RESERVED_QHD
    `else
...
// synthesis translate_on
      ...
      //instantiate and stitch hld_fifo's with reduced width and slicing disabled. last sub-FIFO may not have the full SLICE_WIDTH of data, in which case remainder is used
